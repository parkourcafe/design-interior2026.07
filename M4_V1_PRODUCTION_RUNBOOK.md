...
- **база** — право `execute` на три RPC у роли `authenticated`;
...
```sql
-- 1.1 Миграции доехали. Ожидаемый хвост — ...010000, ...020000, ...030000.
...
Проверка «усечённый результат не получает ложный `complete`» на живом проекте
не воспроизводится: чтобы получить усечение, нужен граф глубже 8 или шире 5000
влияний, и создавать такой в production специально — значит портить данные
ради теста. Она доказана прогоном на настоящем PostgreSQL 16 и 17
(`tests/db5/26_impact_policy_benchmark.sql`): звезда на 6000 даёт
`result_limit` c ровно 5000 сохранёнными, цепочка длиной 14 даёт `depth_limit`,
рассмотрение всех карточек не закрывает заявку, и закрыть её можно только
явным подтверждением архитектора.
...
|---|---|---|
| `calculatedTruncated > 0` | прогон сохранён частично: граф глубже или шире политики | архитектор рассматривает карточки и **подтверждает неполноту** — до этого заявку нельзя закрыть, и это правильно |
