...
export async function POST(
  request: NextRequest,
  context: RouteContext<"/api/projects/[projectId]/imports/[candidateId]/review">,
) {
  ...
  try {
    ...
    if (!integrationProjectIdPattern.test(projectId) || !z.string().uuid().safeParse(candidateId).success) {
    ...
    const data = await new IntegrationConnectionService(requestContext.client)
      .reviewImportCandidate({ candidateId, decision: body.data.decision, target: body.data.target, idempotencyKey });
