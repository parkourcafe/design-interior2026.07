...
const unbindBodySchema = z.object({ reason: z.string().trim().min(1).max(512) }).strict();
...
export async function POST(
  ...
  try {
    ...
    if (!integrationProjectIdPattern.test(projectId) || !z.string().uuid().safeParse(connectionId).success) {
    ...
    const data = await new IntegrationConnectionService(requestContext.client)
      .unbindProjectConnection({ projectConnectionId: connectionId, reason: body.data.reason, idempotencyKey });
