...
import { digestOpaqueToken } from "@/lib/project-intelligence/adapters/storage";
import {
  PROJECTCEO_INVITATION_ACCEPT_CONTRACT_VERSION,
...
export async function POST(request: Request) {
  ...
  if (!isSameOriginMutation(request)) {
    return NextResponse.json(errorBody(requestId, "forbidden"), {
      status: 403,
      headers,
    });
  }
  ...
  const parsed = projectCeoInvitationAcceptSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(errorBody(requestId, "validation_failed"), {
      status: 400,
      headers,
  ...
  if (isProjectCeoLocalFixtureMode()) {
    return NextResponse.json(errorBody(requestId, "operation_unavailable"), {
      status: 409,
      headers,
    });
  }
  try {
    ...
    const tokenDigest = digestOpaqueToken(parsed.data.token);
    const service = new FoundationPostgresAdapter(context.client);
    const mutation = await service.acceptInvitation({
      tokenDigest,
      idempotencyKey: `ui:accept-invitation:${context.identity.userId}:${tokenDigest.slice(2)}`,
    });
