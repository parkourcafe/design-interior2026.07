...
/**
 * Документ живёт на сервере: черновики и чекпойнты — в базе за RLS
 * (/api/layout-studio), подписанные версии — в хранилище projectceo_product.
 * Раньше существовал и браузерный режим (localStorage) для демо-страниц; в
 * объединённом контуре его нет — потерять работу при чистке кэша нельзя.
 */
export function LayoutStudioShell({
  ...
  const publishVersion = async () => {
    ...
    try {
      ...
      // (metadata.warnings входит в подпись).
  ...
  const exportArtifact = async (format: "json" | "svg" | "png" | "glb" | "print") => {
    try {
      ...
      // Имя файла режется из голого hex: подпись хранится как `sha256:<hex>`,
      // а двоеточие в имени файла Windows и URL-кодировщики понимают по-своему.
      const pngArtifactId = `layout-${exactVersion.semanticHash.replace(/^sha256:/, "").slice(0, 16)}-png`;
