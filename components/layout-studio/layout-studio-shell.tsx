...
export function LayoutStudioShell({ initialDocument }: { initialDocument: LayoutDocument }) {
  ...
  const session = useMemo(() => new EditorSession(initialDocument), [initialDocument]); const [state, setState] = useState<EditorState>(() => session.snapshot()); const [versions, setVersions] = useState<LayoutVersion[]>([]); const [checkpoints, setCheckpoints] = useState<LayoutCheckpoint[]>([]); const [showCeiling, setShowCeiling] = useState(false); const [cameraKey, setCameraKey] = useState(0); const [status, setStatus] = useState(ru.layoutStudio.ownerHold);
  ...
  useEffect(() => { if (!state.dirty) return; const timer = window.setTimeout(() => { void repository.saveDraft(state.document).then(() => { session.markSaved(); refresh(); setStatus(ru.layoutStudio.draftSaved); }).catch((error) => setStatus(String(error))); }, 450); return () => window.clearTimeout(timer); }, [state.document, state.dirty, repository, session]);
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (!(event.metaKey || event.ctrlKey) || event.key.toLowerCase() !== "z") return; event.preventDefault(); if (event.shiftKey) session.redo(); else session.undo(); refresh(); }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, [session]);
  ...
  const save = async () => { await repository.saveDraft(state.document); session.markSaved(); refresh(); setStatus(ru.layoutStudio.draftSaved); };
  const checkpoint = async () => { await repository.createCheckpoint(state.document, `Revision ${state.document.stateRevision}`); await hydrateLists(); setStatus(ru.layoutStudio.checkpointCreated); };
  const restore = async (id: string) => { const item = await repository.loadCheckpoint(id); if (!item) return; const result = session.dispatch(command(state.document, "RESTORE_CHECKPOINT", { document: item.content })); if (result.ok) { refresh(); setStatus(ru.layoutStudio.checkpointRestored); } };
  const publish = async () => { const validationBlocking = state.issues.some((issue) => issue.severity === "blocking"); if (validationBlocking) { setStatus(ru.layoutStudio.publishBlocked); return; } const service = new VersionService(repository); const parent = versions.at(-1); await service.publish(state.document, { versionId: `version.${state.document.documentId}.${versions.length + 1}`, parentVersionId: parent?.versionId, reasonCode: "human_publish", reason: "Утверждение версии человеком" }); await hydrateLists(); setStatus(ru.layoutStudio.versionPublished); };
  const exportVersion = async (format: ExportFormat) => { const version = versions.at(-1); if (!version) { setStatus(ru.layoutStudio.publishFirst); return; } const service = new LayoutExportService(repository, { svg: new SvgProjectionAdapter(), glb: new GlbExportAdapter(), png: new BrowserPngExportAdapter(), print: new PrintExportAdapter() }); const artifact = await service.export(version.versionId, format); const blob = new Blob([artifact.bytes], { type: artifact.manifest.mimeType }); const url = URL.createObjectURL(blob); const anchor = window.document.createElement("a"); anchor.href = url; anchor.download = artifact.manifest.filename; anchor.click(); URL.revokeObjectURL(url); setStatus(`${ru.layoutStudio.exactExport} ${version.versionId}`); };
  ...
  return <main style={{ minHeight: "100vh", background: "#f4f1eb", color: "#24211e", padding: 24, fontFamily: "system-ui, sans-serif" }}>
    ...
    <div style={{ maxWidth: 1500, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) 330px", gap: 18 }}>
      <section style={panel}>
        ...
        {state.activeView === "2d" ? <div style={{ overflow: "auto", minHeight: 560, borderRadius: 16, background: "#fbfaf7" }} onClick={(event) => { const target = (event.target as Element).closest("[data-source-id]"); session.select(target?.getAttribute("data-source-id") ?? undefined); refresh(); }} dangerouslySetInnerHTML={{ __html: renderLayoutSvg(state.document, state.selectionId) }} /> : <><div style={{ display: "flex", gap: 8, marginBottom: 8 }}><button style={button} onClick={() => setShowCeiling((value) => !value)}>{showCeiling ? ru.layoutStudio.hideCeiling : ru.layoutStudio.showCeiling}</button><button style={button} onClick={() => setCameraKey((value) => value + 1)}>{ru.layoutStudio.resetCamera}</button></div><LayoutView3D key={cameraKey} document={state.document} selectedId={state.selectionId} showCeiling={showCeiling} /></>}
      </section>
      <aside style={{ display: "grid", gap: 14, alignContent: "start" }}>
        <section style={panel}><h2 style={{ marginTop: 0, fontSize: 17 }}>{ru.layoutStudio.inspector}</h2>{selected ? <div style={{ display: "grid", gap: 10 }}><strong>{selected.label}</strong>{(["xMm", "yMm", "widthMm", "depthMm", "heightMm"] as const).map((key) => <label key={key} style={{ display: "grid", gap: 4, fontSize: 12 }}>{key}<input value={selected[key]} type="number" disabled={selected.locked} onChange={(event) => dispatchObject(selected.id, { [key]: Number(event.target.value) })} style={{ padding: 8, border: "1px solid #ccc", borderRadius: 8 }} /></label>)}{selected.locked && <small>{ru.layoutStudio.lockedObject}</small>}</div> : <p>{ru.layoutStudio.noSelection}</p>}</section>
        <section style={panel}><h2 style={{ marginTop: 0, fontSize: 17 }}>{ru.layoutStudio.validation}</h2><ul style={{ paddingLeft: 18, fontSize: 13 }}>{state.issues.length ? state.issues.map((issue, index) => <li key={`${issue.code}-${index}`} style={{ color: issue.severity === "blocking" ? "#b42318" : "#8a5b00" }}>{issue.severity}: {issue.code}</li>) : <li>{ru.layoutStudio.validationPass}</li>}</ul></section>
