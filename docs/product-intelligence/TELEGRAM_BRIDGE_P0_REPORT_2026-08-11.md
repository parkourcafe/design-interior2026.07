...
**Про SHA, на которых читать зелёный CI.** Полный прогон CI подтверждён на
`25f802e` (#85) и `af188d5` (#86) — там зелены и gates, и DB4 на обеих версиях
PostgreSQL. Для #88 зелены DB4 PG16 и PG17 на `7cc4c0d`; job gates того же
прогона был **отменён** моим следующим пушем, а не упал.

С 11:34 UTC каждый job во всех ветках завершается за две секунды без единого
шага, без логов и с `runner_id: 0` — раннер не назначается. Так падает и
`cycle 7`, у которого стоит `continue-on-error`. Это состояние аккаунта или
инфраструктуры Actions (исчерпанные минуты либо лимит расходов), а не код:
кодом такую картину воспроизвести нельзя. Повторный запуск ничего не изменил.

Поэтому финальные SHA (`4ef6bc0` и последующие правки комментариев и отчёта)
подтверждены **локальным** прогоном: `lint`, `typecheck`, `test` (1302 passed,
2 skipped), `build`, полная цепочка DB4 на PostgreSQL 16 и гонка подключения.
PostgreSQL 17 на этих SHA не перепроверялся — образ в этом окружении
недоступен, а CI не отвечает. **BLOCKED** до восстановления раннеров.
