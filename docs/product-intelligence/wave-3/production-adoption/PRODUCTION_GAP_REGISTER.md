...
## 7. AP1 update — 18 July 2026

Disposable authenticated Supabase gate теперь пройден локально:

- five real local Auth sessions: owner, architect, builder, client, guest;
- Kora registry 209, 1 800 м², 214 physical sources, 85 materialized,
  129 placeholders, one site photo;
- invitation, authenticated reads, distribution/acknowledgement, exact replay,
  change/impact, photo review, milestone acceptance, CSRF and role isolation;
- request-bound dashboard workspace HTTP checks passed;
- `production_changed=false`.

Это закрывает локальную часть PA-B04, PA-B05, PA-B06, PA-B08 и PA-B19 для
disposable evidence. PA-B09 остаётся открытым для отдельного visual browser
run (desktop/mobile screenshots and console evidence); production adoption
остаётся `NO-GO` до fresh production snapshot, clone rehearsal, backup/restore,
secrets, monitoring and human sign-offs.

## 8. Production snapshot — 19 July 2026

Read-only Supabase inspection for `design2026` is recorded in
[PRODUCTION_SNAPSHOT_2026-07-19.md](./PRODUCTION_SNAPSHOT_2026-07-19.md).
The project is healthy on PostgreSQL 17.6.1.141 in `ap-northeast-1`; the
provider migration ledger is empty; all observed public tables have RLS.
Advisors reported the legacy executable `is_studio_member` function, an
unprotected `rate_limits` table, disabled leaked-password protection, missing
FK indexes, and RLS initplan/performance debt. These are findings only and no
production change was made.

The proposed additive SQL and explicit rollback boundaries are documented in
[PRODUCTION_REMEDIATION_PROPOSAL.sql](./PRODUCTION_REMEDIATION_PROPOSAL.sql)
and [CLONE_REHEARSAL_AND_ROLLBACK_PLAN.md](./CLONE_REHEARSAL_AND_ROLLBACK_PLAN.md).
Disposable clone restore and DB2–DB5 rehearsal remain open; production remains
`NO-GO`.

## 9. Local clone rehearsal — 20 July 2026

Полный local Supabase PG17 migration replay, PG16/PG17 matrix, five-session Kora
gate и authenticated Chrome desktop/mobile QA пройдены. FK indexes,
`rate_limits` hardening и RLS initplan candidates проверены с точным rollback.
Это закрывает локальную часть PA-B09 и существенно усиливает PA-R02, но не
является exact provider backup restore: production ledger пуст, legacy
`rate_limits` отсутствует в authoritative migration chain, безопасный bridge
для `is_studio_member` и hosted leaked-password protection остаются открыты.

Итоговое сравнение и условия пересмотра решения зафиксированы в
[LOCAL_CLONE_ADOPTION_DECISION_2026-07-20.md](../../wave-4/ap1/LOCAL_CLONE_ADOPTION_DECISION_2026-07-20.md).
Production adoption остаётся `NO-GO`; production не изменялся.
