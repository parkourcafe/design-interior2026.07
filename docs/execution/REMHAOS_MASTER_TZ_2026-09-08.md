...
| Оркестратор | одна постоянная сессия Claude Code Remote | GitHub: открывать/комментировать PR, сливать docs-only и не-миграционные PR без метки security при зелёном CI и вердикте ревью (Р31); создавать, будить, останавливать дочерние сессии; править только `docs/execution/*`, `docs/audits/wp/*`, канонические транскрипции WP-C | production-секреты; любые операции с `ztnycrchwxqczqbyegnp`; слияние миграций/security/CI; правка подписанных документов; более двух рабочих сессий одновременно |
...
| WP-25 | BUG-05 (а): helper token-scoped + allowlist-тест + черновик принятия риска | 2.5а | нет | 2–3 | W2–W3 | — (Р26 подписывается по черновику этого WP) |
...
|---|---|---|---|---|
| W0 | оркестратор: протокол и шаблон evidence (этот PR), метки в GitHub; #123 сливается по отдельному разрешению владельца от 08.09 (план запуска), далее CI-PR — только владелец | — | — | Р1, Р2, Р10 |
...
security (WP-14, WP-21, WP-23, WP-25, WP-26, WP-33, WP-35, WP-39) и для WP-C; docs-only — gates + ревью оркестратора.
...
| Р31 | Полномочия слияния оркестратора | docs-only и не-миграционные PR без метки security при зелёном CI + ревью; миграции, security, CI, флаги — владелец |
...
2. **Сессия оркестратора.** `create_session` в среде `Default`: репо продукта, `permission_mode: auto`,
   тег `remhaos-orchestrator`, промпт = `docs/execution/ORCHESTRATOR.md`; коннекторы — GitHub (интеграция среды) и Google Drive (heartbeat); Supabase не подключается:
   проверки advisors на стендах выполняет владелец или отдельная сессия по запросу.
3. **Routine.** `create_trigger` с `persistent_session_id` = сессия оркестратора и расписанием из п.1
   (допустимая форма — Routine со свежей сессией на каждое срабатывание в формате Loops HQ: тогда события
   PR получают рабочие сессии, а оркестратор опрашивает GitHub при каждом пробуждении);
