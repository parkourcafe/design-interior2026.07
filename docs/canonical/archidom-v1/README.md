> SUPERSEDED_BY_BRAND_RENAME_2026-07-28. CURRENT_BRAND: RemHaos. Successor package: docs/canonical/remhaos-v1/. Historical content preserved for provenance.

# ARCHIDOM CANONICAL PACKAGE V1.0

Дата freeze: 27.07.2026

## Product Contract
- ARCHIDOM_CHARTER_v0.5_CANONICAL.md

## Technical Architecture
- ARCHIDOM_PLATFORM_ARCHITECTURE_v1.1.md
- ARCHIDOM_ENTITY_CATALOG_v1.md
- ARCHIDOM_GLOSSARY_v1.md
- ARCHIDOM_WORKFLOW_CATALOG_v1.md
- ARCHIDOM_DECISION_LOG_v1.md

## Execution
- ARCHIDOM_EXECUTION_BRIEF_SPRINT_1.md
- ARCHIDOM_READINESS_MATRIX_v1.csv
- ARCHIDOM_READINESS_UPDATE_v1.md

## Governance
После freeze изменения продукта идут через утверждённый Addendum и Decision Log. После спринта создаются Implementation Report и Readiness Update. Новая независимая «финальная архитектура» не создаётся.
