...
**всех** пакетов baseline, Telegram — только документ.

## Что сделано

| Решение | Изменение | Доказательство |
|---|---|---|
| (4) baseline требует передачу | Миграция `20260925100000_projectceo_m3_handoff_gate.sql`: `publish_baseline_atomic(…, handoff_refs jsonb, …)` (6-аргументная сигнатура удалена, список модуля M3 пересобран с заменой ровно этой записи); каждая ссылка — последняя опубликованная ревизия передачи своего пакета (`M2_HANDOFF_STALE`); набор пакетов ссылок = набор пакетов baseline (`M2_HANDOFF_REQUIRED`); design intent передачи входит в decision-refs baseline (`M2_HANDOFF_NOT_IN_BASELINE`); ссылки в append-only `baseline_handoff_refs`; точный replay первым (те же ссылки → прежний результат, другие → `P1110 HANDOFF_REFS_CHANGED`); replay команды, выполненной до гейта, ссылок задним числом не получает | DB4 `81_m3_handoff_gate.sql`: пустые ссылки, пакет без передачи, устаревшая ревизия, чужой пакет, решение не в baseline — каждый своим отказом; позитив + replay + конфликт replay; ссылки неизменяемы (55000) |
| (4) выпуск требует передачу | Триггер `production_package_versions_require_handoff` (продуктовый путь — записи от `pi_table_owner`, т.е. все двери выпуска): у выпускаемого пакета должна быть ссылка в baseline | DB4 81, шаг 8: запись выпуска для пакета без ссылки отклонена `M2_HANDOFF_REQUIRED` |
| (4) приложение | `handoff-refs.ts`: сервер выводит ссылки (последняя передача по каждому активному пакету) из того же authenticated-чтения (A′); `command-service` без передачи → `unavailable` без RPC; `live-read-port` не предлагает публикацию | `tests/release/m3-handoff-gate.test.ts`, `command-service-supported.test.ts`, `live-read-sanitization.test.ts` |
| (4) цепочки | DB4/DB5: фикстура `tests/fixtures/sql/m3_handoff_fixture.sql` (засев) + `m3_handoff_refs_fixture.sql` (чтение, только DB4/DB5). AP5 (`tests/ap5/m3-handoff-fixture.ts`) и `run-five-sessions.zsh` засевают передачу перед каждым baseline | статический тест порядка засевов; DB4/DB5 PG16/17 зелёные |
| (6) Telegram | `docs/canonical/remhaos-v1/REMHAOS_TELEGRAM_CONTOURS_2026-09-25.md` — DRAFT: сравнение контуров A7 и integration с `файл:строка`, риски, предложение единого контракта, вопросы владельцу. Код не менялся | ревью сверило 5 ссылок — верны |

## Прогоны на `5ad43cd`

| Команда | Результат |
|---|---|
| lint | exit 0 (0 ошибок, 15 предупреждений — как на `737795d`) |
| typecheck / build | exit 0 / exit 0 |
| test | **260/260 файлов, 2346/2346** |
| test:db4 PG16 / PG17 | exit 0, `DB4_M3_HANDOFF_GATE_OK` и все пять маркеров харнесса |
| test:db5 PG16 / PG17 | exit 0, `DB5_EXECUTION_HARNESS_OK` |
| AP5, run-five-sessions | **не запускались** — среда (см. §4 выше); первая проверка — CI на PR |

## Независимое ревью

APPROVE WITH NITS, блокеров нет; обходов гейта не найдено (сырой `publish_project_baseline` отозван у ролей,
все выпуски пишут через `publish_production_package_version` владельца `pi_table_owner`). Исправлено: F1
(replay довоенной команды пришивал ссылки) — medium; F2, F3, F6, F7, F8, F10. Оставлено и вынесено владельцу:
F4 — база проверяет «последнюю ревизию этой передачи», но не «самую свежую передачу пакета» (приложение
выбирает самую свежую); F5 — вхождение design intent проверяется по baseline проекта, выборы/approved commit
передачи не сверяются; F9 — кнопка выпуска для baseline, опубликованного до гейта, предлагается, база её
честно отклоняет.

## Отклонение от согласованного плана

AP5 и run-five-sessions получают передачу **фикстурой одноразовой базы**, а не полным путём M2 в браузере
(три варианта планировки, цены, согласование клиентом, approved commit). Полный путь M2 доказывают DB4 33 и
внешний раннер AP6; писать его в AP5 вслепую, без возможности запустить AP5 здесь, означало бы серию красных
прогонов CI. Полный путь M2 в браузерной цепочке — отдельная задача по решению владельца.
