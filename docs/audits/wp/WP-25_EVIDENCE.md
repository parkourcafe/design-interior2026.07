...
конкретные route paths после grep-first. Риск не подписан и не принят.
...
[ИЗВЛЕЧЕНО] Черновик Р26 имеет статус НЕ ПОДПИСАНО / РИСК НЕ ПРИНЯТ.
...
не проверяется. Зафиксировано в черновике как остаточный риск; семантика
авторизации в WP-25 не менялась.
[ИНТЕРПРЕТИРОВАНО] Пакет не закрыт до подписанного владельцем Р26,
security blind review и обязательного CI. Коммит/push/PR не выполнялись.
...
[ИЗВЛЕЧЕНО] CI отсутствует, Р26 не подписан. Это локальный кандидат;
риск service-role клиентов не принят и production acceptance не наступил.
