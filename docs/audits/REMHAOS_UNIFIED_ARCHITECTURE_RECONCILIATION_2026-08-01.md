...
## Поправка 10.08.2026 — только колонка репозитория

Снимок выше датирован 1 августа и с тех пор устарел **в части репозитория**.
Колонка production **не трогается**: свежего read-only снимка у этой правки
нет, и делать утверждения о production без него нельзя — вывод
`PRODUCTION_ADOPTION=NO-GO` и требование свежего снимка остаются в силе.

Что изменилось в репозитории (всё влито в `main`):

| Строка снимка | Было 01.08 | Стало 10.08 |
|---|---|---|
| M2 | «room/variants/material-budget vertical slice отсутствует» | Порт Layout Studio влит (#71): черновики, привязка к пакету, бюджет варианта, DXF, публикация версий через контур; контракт документа разведён на 0.1/0.2 в реестре версий |
| M3 | «publish_baseline есть; UI/read помечает baseline unavailable; source/review/release/handover commands не закрыты» | M3 в объёме P0 влит (#72): лист документации, комплектность, персистентность `projectceo_m3`, чтение v7, intake-команды. Командная поверхность AP3 закрыта полностью (#73): `publish_release` — настоящая команда, handover остаётся воркерным путём по AP3 §10 |
| Ворота AP | AP1 закрыт, AP2/AP3 открыты | AP1–AP4 закрыты; открытыми остаются AP5 (браузерный E2E по ролям), AP6 (внешний пакет) и AP7 (коммерческая валидация) |

Что **не** изменилось и остаётся верным: два контура по-прежнему не один
deployment, migration history не сведена, `projectceo_*` в production не
применены, локальный PASS не является production evidence. Реестр конфликтов
(`REMHAOS_CONFLICT_REGISTER_2026-08-01.csv`) в части production-строк
актуален; строки CR-…-04 и CR-…-05 по колонке репозитория закрыты, по колонке
production — нет.
