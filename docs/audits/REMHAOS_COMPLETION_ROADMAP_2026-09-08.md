...
Оценка Трека 0: 4–6 РС инженера + 2–3 часа владельца в дашбордах.
...
3. [eng] **Reconciliation PR** — каталог `production-adoption/reconciliation-2026-09/`:
   ...
   - `tests/ap1/environment/adopt-production.zsh` — новый операторский скрипт (НЕ трогать `bootstrap-disposable.zsh`: его `reject_production` — контракт): `supabase/roles.sql`, миграции в порядке `migration-ledger.sha256` с проверкой хеша, `verify-db.sql`, `apply-hosted-role-precondition.sql`; требует `AP1_APPROVAL_RECORD` и allowlist ref; контрактный тест — отказ без записи одобрения и на чужом ref;
...
4. [eng + owner] **Репетиция на клоне**: новый disposable-проект → восстановить prod backup (`pg_restore`; Supabase branch клоном не является) → `adopt-production.zsh` → `POST_VERIFY` → `npm run provision:ap1` → hosted AP5 (`ci.yml` job `hosted-staging`, секреты на клон) 27/27, 0 skipped → legacy-смоук на реальных данных клона → клон удалить (живые PII). Итерировать п.3 до зелёного.
...
9. [owner исполняет] **Окно на prod**: freeze мержей → backup/PITR timestamp → `adopt-production.zsh` → Data API exposed schemas (список, который пиннит `tests/ap1/environment/environment.contract.test.ts`) → reload schema cache → `verify-runtime.mjs` против prod → advisors errors=0 → deploy RC → смоук legacy M1 тестовым дизайнером → 48 ч наблюдения → §14 execution record. Итог ledger = 23 + 1 + 90 = 114.
...
1. **PR #124** (`codex/m1-project-workspace-contracts`, HEAD `1a7ae0b`): после 0.1 — прогон на точном HEAD (gates, DB4×2, DB5×2, AP5, Claude review); после мержа #123 — rebase; ledger 91; merge только после решения Р11 (автор PR сам это требует). 1 РС.
...
4. **Мёртвый контур `acknowledgeImpactTruncation`**: удалить метод адаптера в `adapters/postgres/execution.ts` (BUG-06), поля `allImpactsReviewed`/`truncationAcknowledged` из `ImpactReviewMutation` (BUG-07), `unacknowledgedTruncatedRunId` в `live-read-port.ts` (BUG-08); **оставить** kind в `command-contract.ts:261`, `not_authorized` в `action-registry.ts:79`, `execution-flag.ts:106`, `live-read-port.ts:1320` — AP5 звено 12 утверждает отказ `operation_unavailable` до базы; «Поправка» к §7.2 `REMHAOS_M4_V1_PRODUCTION_READINESS_AUDIT_2026-08-12.md` (не переписывать). 1 РС.
...
или `INACTIVE`; `get_advisors(prod, security)` → нет findings по `market_harvest` и `rate_limits`,
leaked-password включена; `Railway.list-services(remhaos-market-harvest)` → сервиса нет;
`github.list_pull_requests` → #122 closed, #123 merged; `actions_list` → прогоны CI на PR-HEAD
длятся минуты и зелёные; `grep -rn ukkzasfsmannjprfkaxp .` → 0; `git branch -r | wc -l` ≤ 10.
