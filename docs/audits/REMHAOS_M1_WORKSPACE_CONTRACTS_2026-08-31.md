...
- [ИЗВЛЕЧЕНО] Implementation commit: `ffdfdcb` (`feat: expose M1 project facts and approval lifecycle`).
...
- [ИЗВЛЕЧЕНО] No migration file was changed. The source checkout contains 90 migration files.
...
- [ИЗВЛЕЧЕНО] Draft, submitted, approved and rejected states are shown from the live projection; command targets are server-selected.
...
- [ИЗВЛЕЧЕНО] `npm test`: 196 files passed; 1,570 tests passed; 10 existing tests skipped.
- [ИЗВЛЕЧЕНО] `npm run build`: pass; Next.js production build completed.
- [ИЗВЛЕЧЕНО] `npm run test:db4`: pass on `postgres:16-alpine`, including platform facts, approval requests, M1 RLS, concurrency and restart replay.
- [ИЗВЛЕЧЕНО] `npm run test:db5`: pass on `postgres:16-alpine`, including execution security, concurrency and restart replay.
- [ИЗВЛЕЧЕНО] Changed-file and staged diff scans found no bearer, JWT, password, private-key or provider-key pattern.

### CI_EVIDENCED

- [ИЗВЛЕЧЕНО] None yet. This report was produced before the PR checks ran.
...
- [ИЗВЛЕЧЕНО] Canonical `npm run test:ap5` did not start because `NEXT_PUBLIC_SUPABASE_URL` was not present in the environment. No credential was invented or read from a secret file.
