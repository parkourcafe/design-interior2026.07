...
DEC-031) — **только в объёме P0**. Telegram становится основным внешним каналом
рабочей переписки проекта; RemHaOS остаётся системой официальных решений,
полномочий, версий, изменений и доказательств.

Мост — **один горизонтальный adapter** в `Integration Gateway → Messaging`, а не
Module 5 и не отдельный бот на каждый модуль (DEC-001 подтверждается, не
обходится). Отсюда правила, действующие с даты подписи:

* Telegram-код не пишется внутри M1, M2, M3 и M4; `telegram_chat_id` и
  `telegram_user_id` не появляются в доменных таблицах модулей;
* webhook не пишет напрямую в business tables и не выполняет человеческих RPC
  под `service_role`; команды из текста сообщения не исполняются; LLM в контуре
  моста не получает tools и mutations;
* сообщения, фото, документы и AI-выводы создают **только неподтверждённых
  кандидатов** в Project Inbox — официальным объектом кандидат становится
  человеческой командой RemHaOS после входа и явного подтверждения;
* мост не переносит полномочия: членство в группе не создаёт аккаунт или роль,
  «получил» не выполняет `acknowledge_release`, фото не является evidence и
  приёмкой, callback и deep link не выполняют mutation;
* бот **не может** создать человеческую группу — её создаёт или выбирает
  пользователь и добавляет бота через `startgroup`; текст действия в UI ровно
  «Создать или подключить Telegram-чат», автоматическое создание чата обещать
  нельзя;
* связь Telegram-аккаунта строится только по стабильному числовому user ID,
  никогда — по username, телефону или отображаемому имени;
* гарантия доставки уведомлений — `at-least-once + внутренняя дедупликация`;
  exactly-once и синхронизация удалений Telegram не обещаются.

Объём P0 — один вертикальный сценарий M3 → M4 (`distribute_release` →
уведомление → защищённый переход → `acknowledge_release` сессией получателя →
сообщение строителя → `change_request_candidate` → человек → `create_change`).
M1, M2 и прочие сценарии M3 в P0 не подключаются. Внутренний мессенджер RemHaOS
не строится.

Флаг `REMHAOS_TELEGRAM_BRIDGE_ENABLED` выключен по умолчанию: выключенный мост
не принимает, не классифицирует и не отправляет сообщения. A7 правит Charter §12
ровно настолько, чтобы разрешить разработку и тестирование в local, CI и
закрытом staging с тестовыми участниками, и **не разрешает** production-включение,
массовый приём реальной переписки третьих лиц, обход российского data-plane
гейта, признание вопроса 152-ФЗ закрытым и M4 Increment 2. Production — отдельный
OWNER GO после legal/data-plane/consent/retention гейта. Цикл 7 остаётся
открытым.
