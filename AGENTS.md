...
- универсальную чертёжную систему общего назначения: геометрический редактор
  существует только для проектов внутри RemHaOS и отдельно не продаётся; не
  строим чертёжные возможности, не связанные с фиксацией решения, дисциплины вне
  интерьера и ремонта, собственный формат обмена как продукт. Сам редактор
  планировок с 2D/3D и вводом геометрии мышью **разрешён** решением ADR-0006;
