...
**Recovery заблокированного прогона — DEC-037 LOCKED (OWNER DECISION
17.08.2026,
`docs/canonical/remhaos-v1/REMHAOS_OWNER_DECISION_M4_V1_COVERAGE_AND_RECOVERY_2026-08-17.md`).**
Тот же документ ратифицирует DEC-034 (контракт покрытия, вариант А: blocked —
только durable-метаданные, дверей у `authenticated` ровно две) и DEC-035
(`maxDepth = 7` зафиксирован записью владельца) как явные последующие решения
поверх DEC-033. Recovery — четыре элемента (миграция `20260817010000`):
ранняя остановка расчёта ограниченным счётчиком пар до материализации
результата; durable alert (строка прогона + признак и рекомендация rescope в
поверхности чтения + отдельное поле `calculatedBlocked` в отчёте воркера);
rescope как человеческий путь (существующая `submit_change_request`, новых
дверей нет); superseding recalculation как системный путь — пересчёт разрешён
РОВНО для blocked-прогона устаревшей версии политики, новый прогон вытесняет
старый (`superseded_at`), «один активный прогон на заявку» держит частичный
уникальный индекс, ключ идемпотентности воркера включает версию политики.
Прогоны `complete`/`partial_depth` не вытесняются никогда; append-only
триггер прогонов разрешает ровно один односторонний переход (активный →
вытесненный) при побайтово неизменном остальном. Несмерженная миграция
DEC-034 приведена к безопасности на непустой `impact_runs` (backfill трёх
форм до `NOT NULL`, §5 решения); доказательства — харнесс апгрейда
населённой базы `tests/db4/run-impact-upgrade.zsh` и сценарий
`tests/db5/32_impact_recovery_dec037.sql`, оба в обязательном CI.
