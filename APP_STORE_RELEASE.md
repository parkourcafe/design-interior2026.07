# APP_STORE_RELEASE — подготовка RemHaOS к Apple App Store

Обновлено: 30.07.2026. Цель: первая публикация RemHaOS для iPhone через
App Store Connect и TestFlight.

## Статус

**iOS source target подготовлен, submission пока NO-GO.** Создан Capacitor/Xcode target с bundle ID
...
Публичное имя изменено на RemHaOS. Bundle ID намеренно сохранён: имя приложения
можно менять независимо, а после загрузки build Bundle ID становится неизменяемым.
...
5. Проверить production-страницу поддержки с реальными контактами. Support URL:
   `https://www.remhaos.com/support`.
...
- Display name: `RemHaOS`
- Version: `1.0.0`; build: `1`
- Minimum iOS: 15.0
- Orientation: portrait как основной; iPad включать только после отдельного QA
- Universal Links: `applinks:www.remhaos.com` для auth/public links
- Associated Domains требуют рабочий
  `https://www.remhaos.com/.well-known/apple-app-site-association`
...
а RemHaOS превращает ответы в паспорт проекта, предлагает возможные риски,
...
Первый релиз RemHaOS: клиентский бриф, паспорт проекта, карточки рисков,
расчёт стоимости и коммерческое предложение в одном пресейл-контуре.

**Privacy Policy URL:** `https://www.remhaos.com/legal/privacy`

**Support URL:** `https://www.remhaos.com/support`
...
RemHaOS is a business workflow tool for interior designers. The public demo is
...
Удаление аккаунта реализовано; перед submission требуется device QA этого потока.
...
1. Проверить юридический блок и account deletion на production и устройстве.
2. Убедиться в App Store Connect, что app record использует `space.arhidom.ios`.
3. Проверить Universal Links, privacy manifest, icons и launch screen.
4. Прогнать signed release build и ручной device QA.
...
- подтверждение App Store Connect record для `space.arhidom.ios`
