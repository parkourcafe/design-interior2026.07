...
| Скриншоты для витрины (в манифесте) | `app/manifest.ts` screenshots | ✅ |
...
|---|---|---|
| Иконка | 512×512 PNG | 512×512 PNG |
| Feature graphic / баннер | 1024×500 PNG/JPG | 1080×607 (баннер) |
| Скриншоты телефона | 2–8 шт, мин. 320px, 16:9/9:16 | 3–8 шт |
...
- **Иконку 512×512** можно взять с сайта: `https://arhidom.space/icons/512`
  (сохранить как PNG).
- **Скриншоты**: снять на телефоне реальные экраны arhidom.space (главная,
  демо-бриф, demo-КП, кабинет) ИЛИ переиспользовать брендовые визуалы (они уже
  в манифесте, form_factor narrow/wide). Для стора лучше настоящие экраны
  приложения — снять после установки TWA.
...
3. Категория: «Бизнес» / «Производительность». Возраст: 0+/6+.
...
> RuStore принимает TWA. Если модерация попросит «больше нативности» — это
> редкий кейс; обычно TWA с рабочим assetlinks и офлайн-экраном проходит.
...
- **Медиа на свой хостinг** (снять зависимость от CDN Higgsfield):
