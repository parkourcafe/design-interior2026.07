...
import { assertOwnedContainer } from "../../lib/integration-gateway/r1-sandbox/profile";
...
async function main() {
  ...
  try {
    ...
    supervisor = spawn(process.execPath, ["--import", "tsx", resolve("scripts/r1-sandbox/create-interruption-supervisor.ts"), registryPath, imageId, sourceHash, "wait", profile, endpoint, expectedDaemonId], { detached: true, stdio: "ignore", env });
    ...
    let operationId: string | undefined; let containerId: string | undefined;
    for (let i = 0; i < 80; i++) {
      const own = registry.live(host.daemonId).find(row => row.supervisorPid === supervisor?.pid);
      if (own && own.containerId === null && own.state === "create_inflight") {
        const listed = await docker.command(["ps", "--all", "--no-trunc", "--filter", `name=^/${own.name}$`, "--format", "{{.ID}}"]);
        const candidate = listed.stdout.trim();
        if (listed.code === 0 && /^[a-f0-9]{64}$/.test(candidate)) {
          const observation = await docker.inspect(candidate);
          if (observation) { assertOwnedContainer(own, observation); operationId = own.operationId; containerId = candidate; break; }
        }
      }
    ...
    process.stdout.write(JSON.stringify({ level: "synthetic-create-interruption-v1", operationId, containerId, supervisorPid: supervisor.pid,
      watchdogPid: watchdog.pid, watchdogSurvived: survived, elapsedMs, settled, absent, recoveredExactId: registry.read(operationId)?.containerId === containerId, withinBound: elapsedMs <= 14_000 }) + "\n");
