...
import { runReadonlyNegativeControl } from "../../lib/integration-gateway/r1-sandbox/executor";
...
async function main() {
  ...
  if (process.argv.length !== 8 || !registryPath || !imageId || !probeSourceSha256 || !profile || !endpoint || !expectedDaemonId) throw new Error("sandbox_arguments_invalid");
  ...
  try {
    ...
    try {
      const result = await runReadonlyNegativeControl({ registry, docker, colimaExecutable: "/opt/homebrew/bin/colima", profile, expectedDaemonId, imageId, probeSourceSha256 }, new AbortController().signal);
