...
-- Пустая на момент применения таблица (см. `20260812020000`: единственная
-- строка, которую миграция могла бы вставить, откатывается тем же guard-
-- блоком через перехваченное исключение) — NOT NULL можно поставить сразу
-- следующим `alter`, без backfill.
alter table projectceo_m4.impact_runs
