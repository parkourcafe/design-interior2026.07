...
begin;
create function projectceo_read_api.get_pdf_dwg_source_pair_confirmation(project_id uuid,package_id uuid,confirmation_id uuid)
returns jsonb language plpgsql volatile security definer set search_path='' as $f$
#variable_conflict use_variable
declare ctx record; receipt projectceo_foundation.pdf_dwg_source_pair_confirmations%rowtype;
...
begin
 ...
 select r.* into receipt from projectceo_foundation.pdf_dwg_source_pair_confirmations r where r.organization_id=ctx.organization_id and r.project_id=project_id and r.package_id=package_id and r.confirmation_id=confirmation_id;
 if not found then perform projectceo_foundation._raise('P1104','not_found','{}'); end if;
 return jsonb_build_object('contractVersion','r1-source-pair-read/1','result',projectceo_foundation._pdf_dwg_pair_result(receipt));
end $f$;
alter function projectceo_read_api.get_pdf_dwg_source_pair_confirmation(uuid,uuid,uuid) owner to pi_table_owner;
revoke all on function projectceo_read_api.get_pdf_dwg_source_pair_confirmation(uuid,uuid,uuid) from public,anon,authenticated,service_role,pi_human_executor,pi_worker_executor;
...
declare before_m3 text[]:=projectceo_platform._module_signatures('m3'); before_m4 text[]:=projectceo_platform._module_signatures('m4_increment_1');
 signature text:='projectceo_read_api.get_pdf_dwg_source_pair_confirmation(uuid, uuid, uuid)'; definition text; owner_id oid; acl aclitem[];
begin
