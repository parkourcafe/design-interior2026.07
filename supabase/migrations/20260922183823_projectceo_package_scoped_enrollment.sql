...
declare
  ...
  v_user uuid;
  v_role text;
  v_package projectceo_foundation.project_packages%rowtype;
begin
  ...
  if members is null or jsonb_typeof(members) <> 'array' or jsonb_array_length(members) > 100 then
  ...
  if exists (
    ...
    where jsonb_typeof(member) is distinct from 'object'
      or jsonb_typeof(member->'userId') is distinct from 'string'
      or jsonb_typeof(member->'role') is distinct from 'string'
      or member->>'role' is null
      or member->>'role' not in ('architect', 'builder', 'client_approver')
  ) then
    perform projectceo_foundation._raise('P1111', 'validation_failed', '{"field":"members"}'::jsonb);
  end if;
  begin
    if exists (
      select 1 from jsonb_array_elements(members) member
      group by (member->>'userId')::uuid
      having count(*) > 1
    ) then
      perform projectceo_foundation._raise('P1111', 'validation_failed', '{"field":"members.duplicateUserId"}'::jsonb);
    end if;
  ...
  if package_stable_key is null or package_name is null
     or btrim(package_stable_key) <> package_stable_key or char_length(package_stable_key) not between 1 and 160
  ...
  from projectceo_foundation.project_packages existing
  where existing.organization_id = v_org and existing.project_id = project_id and existing.id = package_id
  for update;
  if not found or v_package.stable_key <> package_stable_key or v_package.kind <> 'work_package'
     or v_package.parent_package_id is distinct from project_id or v_package.name <> package_name then
    perform projectceo_foundation._raise('P1109', 'scope_conflict', '{"reason":"PACKAGE_IDENTITY_MISMATCH"}'::jsonb);
  end if;
  if v_package.status <> 'active' then
    perform projectceo_foundation._raise('P1109', 'scope_conflict', '{"reason":"PACKAGE_INACTIVE"}'::jsonb);
  end if;

  insert into project_intelligence.organization_members (organization_id, user_id, role, status)
  values (v_org, v_actor, 'owner', 'active') on conflict do nothing;
  insert into projectceo_foundation.project_memberships
    (organization_id, project_id, user_id, role, status)
  values (v_org, project_id, v_actor, 'owner_lead', 'active') on conflict do nothing;
  ...
  from projectceo_foundation._role_capabilities('owner_lead') rc on conflict do nothing;
  ...
  for v_member in select value from jsonb_array_elements(members) loop
    ...
    if exists (
      select 1 from projectceo_foundation.package_memberships existing
      where existing.organization_id = v_org and existing.project_id = project_id
        and existing.package_id = package_id and existing.user_id = v_user and existing.role <> v_role
    ) then
      perform projectceo_foundation._raise('P1109', 'scope_conflict', '{"reason":"PACKAGE_MEMBER_ROLE_REPLACEMENT_REQUIRED"}'::jsonb);
    end if;
    insert into project_intelligence.organization_members (organization_id, user_id, role, status)
    values (v_org, v_user, 'member', 'active') on conflict do nothing;
    insert into projectceo_foundation.package_memberships
      (organization_id, project_id, package_id, user_id, role, status)
    values (v_org, project_id, package_id, v_user, v_role, 'active') on conflict do nothing;
    insert into projectceo_foundation.package_member_capabilities
      (organization_id, project_id, package_id, user_id, capability)
    select v_org, project_id, package_id, v_user, rc.capability
