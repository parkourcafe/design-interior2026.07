...
begin
  ...
  -- ОБЛАСТЬ ПАКЕТА. `project_baseline_refs` не хранит package_id: состав
  -- baseline описан ревизиями, а принадлежность ревизии пакету живёт в другой
  -- таблице. Значит отдать «состав, суженный до пакета» это чтение не может — а
  -- отдать полный состав участнику с доступом к ОДНОМУ пакету значит показать
  -- ему идентификаторы ревизий соседних пакетов. Это утечка, и первая редакция
  -- миграции её содержала.
  --
  -- Поэтому поле появляется только у чтения областью проекта. Сборщику версии
  -- этого достаточно: версия строится для корневого пакета, а корневой пакет
  -- виден только тому, у кого область — проект (`ROOT_PACKAGE_REQUIRES_FULL_BASELINE`).
  -- Участник пакета не теряет ничего, что имел бы право увидеть.
