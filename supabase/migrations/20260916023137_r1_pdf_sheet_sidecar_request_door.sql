begin;
create function projectceo_api.bind_pdf_dwg_sheet_sidecar(project_id uuid,package_id uuid,confirmation_id uuid,sheet_id text,sheet_revision_id text,pdf_page_index bigint,pdf_crop jsonb,rotation_degrees integer,units text,page_to_preview_transform jsonb,reason text,idempotency_key text)
returns jsonb language sql volatile security definer set search_path='' as $f$
 select projectceo_foundation._bind_pdf_dwg_sheet_sidecar(project_id,package_id,confirmation_id,sheet_id,sheet_revision_id,pdf_page_index,pdf_crop,rotation_degrees,units,page_to_preview_transform,reason,idempotency_key);
$f$;
alter function projectceo_api.bind_pdf_dwg_sheet_sidecar(uuid, uuid, uuid, text, text, bigint, jsonb, integer, text, jsonb, text, text) owner to pi_table_owner;
revoke all on function projectceo_api.bind_pdf_dwg_sheet_sidecar(uuid, uuid, uuid, text, text, bigint, jsonb, integer, text, jsonb, text, text) from public,anon,authenticated,service_role,pi_human_executor,pi_worker_executor;
...
declare before_m3 text[]:=projectceo_platform._module_signatures('m3'); before_m4 text[]:=projectceo_platform._module_signatures('m4_increment_1');
 signature text:='projectceo_api.bind_pdf_dwg_sheet_sidecar(uuid, uuid, uuid, text, text, bigint, jsonb, integer, text, jsonb, text, text)'; definition text; owner_id oid; acl aclitem[];
begin
