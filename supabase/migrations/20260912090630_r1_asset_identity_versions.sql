...
alter table projectceo_foundation.external_asset_events add foreign key (organization_id, actor_user_id) references project_intelligence.organization_members (organization_id, user_id) on delete restrict;
alter table projectceo_foundation.external_asset_events add constraint external_asset_events_actor_shape check ((actor_type='human' and actor_user_id is not null) or (actor_type='system' and actor_user_id is null));
