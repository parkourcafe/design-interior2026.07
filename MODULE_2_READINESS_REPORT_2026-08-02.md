...
- The full M2 workspace (rooms, variants, real materials/items, budget frame and
  client approval handoff) has not passed a production-shaped clone rehearsal.
...
1. Build a disposable production-shaped clone from the schema/ACL/RLS snapshot.
2. Add and test only the smallest additive provenance projection/bridge on that
