...
export function validateR1Archive(input: { readonly compressedBytes: number; readonly expandedBytes: number; readonly entries: readonly { path: string; kind: "file" | "directory" | "symlink"; encrypted?: boolean }[] }) {
  if (!Number.isSafeInteger(input.compressedBytes) || !Number.isSafeInteger(input.expandedBytes) || input.compressedBytes <= 0 || input.expandedBytes <= 0 || input.entries.length > R1_WORKER_LIMITS.maxArchiveEntries) throw new Error("r1_archive_limits_invalid");
  ...
  for (const entry of input.entries) {
    ...
    if (!normalized || normalized.startsWith("/") || /^[a-z]:/i.test(normalized) || normalized.split("/").some((part) => part === "" || part === "." || part === "..") || !["file", "directory"].includes(entry.kind) || entry.encrypted || names.has(normalized)) throw new Error("r1_archive_entry_rejected");
