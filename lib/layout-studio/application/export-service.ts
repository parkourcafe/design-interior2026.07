...
interface ExportableLayoutVersion {
  ...
  documentId: string;
  /**
   * Подпись версии в записи хранилища. Каноническая форма — `sha256:<hex>`;
   * голый hex принимается для совместимости со старыми записями.
   */
  semanticHash: string;
  content: LayoutDocument;
}

/** Каноническая форма подписи в записях версий: `sha256:` + 64 hex-символа. */
const SEMANTIC_HASH_PREFIX = "sha256:";

/**
 * Убрать префикс алгоритма и получить голый hex.
 *
 * Экспортёр сравнивает подпись с результатом semanticHash(), который возвращает
 * hex без префикса, и режет первые 16 символов на имя файла. И то и другое
 * должно работать одинаково для обеих форм записи, иначе выгрузка версии из
 * серверного хранилища падала бы на SEMANTIC_HASH_MISMATCH, а имя файла
 * получало бы в середине двоеточие.
 */
function bareSemanticHash(value: string): string {
  return value.startsWith(SEMANTIC_HASH_PREFIX)
    ? value.slice(SEMANTIC_HASH_PREFIX.length)
    : value;
}
...
export class LayoutExportService {
  ...
  async exportVersion(
    ...
    const storedHash = bareSemanticHash(version.semanticHash);
    const actualSemanticHash = await semanticHash(version.content);
    if (actualSemanticHash !== storedHash) {
      throw new LayoutExportError(
    ...
    // Предупреждения берутся из самого документа, а не из метаданных публикации.
    // Они входят в подпись, поэтому их нельзя ни потерять по дороге, ни дописать
    // задним числом: тот, кто публикует версию, не может решить, что оговорку
    // «размеры со слов, обмера не было» в выгрузку класть не нужно.
    const warnings = [...version.content.metadata.warnings];
    ...
    // Имя артефакта режется из голого hex: `sha256:` в имени файла дал бы
    // двоеточие, которое Windows и URL-кодировщики понимают по-своему.
    const artifactId = `layout-${storedHash.slice(0, 16)}-${format}`;
