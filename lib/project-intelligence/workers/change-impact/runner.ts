 ...
 *     не из часов и не из случайности.
  ...
  *     `IMPACT_ALREADY_CALCULATED`, либо `idempotency_conflict`, либо сырой
  *     `unique_violation` частичного индекса — все три исхода для воркера
  *     успех, а не ошибка.
...
function outcomeFromRace(error: unknown): "already_present" | "stale_state" | null {
  ...
  // Сырой unique_violation (`23505`) без именованной причины — это частичный
  // уникальный индекс `m4_impact_runs_one_active_key` (DEC-037): сосед успел
  // вставить активный прогон между нашей exists-проверкой и вставкой. Через RPC
  // путь этот случай закрыт `FOR UPDATE`-сериализацией и P1110 — здесь
  // защита в глубину на случай, когда до индекса доходит тот, кто блокировку
  // не брал. Для очереди это тоже «уже готово», а не повод для пяти попыток.
  if (error.code === "internal_error" && error.sqlstate === "23505") {
