...
--
-- ЧТО РОЛЬ НЕ МЕНЯЕТ. Она не заводится миграцией и в постоянной схеме её нет:
-- живёт только внутри одноразового контейнера DB5, создаётся харнессом и
-- исчезает вместе с контейнером. PostgREST её не видит — она `nologin` и не
-- перечислена ни в `supabase/config.toml`, ни в цепочке миграций (проверяется
-- статически в `tests/db5/static-boundary.test.ts`). Постоянные гранты этот
-- файл не трогает.
--
...
begin
  ...
  -- 3. Права на функции — ровно шесть, и ровно те. Проверяется ЯВНАЯ запись в
  --    ACL, а не `has_function_privilege`: последний вернул бы true и там, где
  --    право досталось роли через `PUBLIC`, то есть скрыл бы утечку.
  ...
  where acl.grantee = v_role.oid;
  if v_count <> 6 then
    raise exception 'DB5_TEST_ROLE_FUNCTION_GRANT_COUNT:%', v_count;
  end if;
