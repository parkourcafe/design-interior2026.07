...
begin
  ...
  -- прогон был бы тупиком, который нельзя закрыть никогда (DEC-034 закрыла
  -- её навсегда позже, но не удалила саму функцию). Стало 19: DEC-036 (OWNER
  -- REVIEW 12.08.2026) добавила `record_change_impact_worker_failure`
  -- (`20260813030000`) — дверь воркера, записывающая durable отказ.
  -- Двадцатая функция без нового решения обязана уронить прогон.
  if v_count <> 19 then
