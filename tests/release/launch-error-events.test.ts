...
const state = vi.hoisted(() => ({ failure: "", rows: [] as Record<string, unknown>[], known: true, llmOk: true }));
vi.mock("@/lib/intake", () => ({ getProjectByIntakeToken: async () => state.known ? { id: "project", designer_id: "designer", status: "created" } : null }));
...
vi.mock("@/lib/supabase/token-scoped", () => ({ createScopedServiceClient: () => ({
  ...
  from: (table: string) => {
    ...
    const result = () => ({ data: table === "proposals" ? { id: "proposal", project_id: "project", status: "sent" } : table === "projects" ? { designer_id: "designer" } : [], error: state.failure === `${table}:${operation}` ? { message: "private db details" } : null });
    const query = {
...
beforeEach(() => { state.failure = ""; state.rows = []; state.known = true; state.llmOk = true; });
