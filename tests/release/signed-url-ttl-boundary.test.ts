...
describe("signed URL TTL boundary", () => {
  it("does not pass an excessive or unverified TTL outside the storage adapter", () => {
    ...
    // Полный проход компилятором по app/lib/components с проверкой типов
    // занимает 2–3,5 с, а под нагрузкой (параллельный прогон, только что
    // выполненный npm ci) доходил до 6,3 с и падал на стандартных 5 с
    // vitest. Проверки от бюджета времени не зависят — поднят только он.
  }, 20_000);
});
