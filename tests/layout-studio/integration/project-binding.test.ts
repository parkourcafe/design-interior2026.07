...
describe("Планировки внутри проекта: границы доступа", () => {
  ...
  it("конфликты отдаются как 409, а не как успех", () => {
    const source = read(API_ROUTE);
    // VERSION_IMMUTABLE здесь больше не живёт: публикация версий ушла из этого
    // роута в command API объединённого контура (publish_m2_layout_version).
    for (const code of ["STATE_STALE", "CHECKPOINT_IMMUTABLE", "DOCUMENT_EXISTS"]) {
      expect(source, `${code} не отображён в HTTP-код`).toMatch(
        new RegExp(`${code}:\\s*409`),
      );
    }
    expect(source, "публикации версий в этом роуте быть не должно").not.toMatch(
      /publishVersion/,
    );
  });
});
