...
describe("LS-060: exact-version JSON/SVG export", () => {
  ...
  it("builds the print summary strictly from Version A after Version B is live", async () => {
    const repository = new MemoryLayoutRepository();
    // Предупреждения живут в metadata самого документа и входят в подпись —
    // хранилище объединённого контура не доверяет метаданным публикации.
    const documentA = makeSimpleRoom();
    documentA.metadata.warnings = [versionAWarning];
    const versionA = await repository.publishVersion(documentA, {
    ...
    expect(moved.ok).toBe(true);
    moved.document.metadata.warnings = ["Предупреждение только версии B"];
  ...
  it.each([
    ...
    async (format, extension) => {
      ...
      // Предупреждение — в подписанном документе, не в метаданных публикации.
      document.metadata.warnings = [versionAWarning];
