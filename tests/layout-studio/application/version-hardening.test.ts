...
/**
 * LS-050: защита черновиков и версий в эталонном хранилище.
 *
 * Первоисточник этих сценариев — тест BrowserLayoutRepository из ветки движка.
 * Браузерное хранилище в объединённый контур не переехало (демо-страницы
 * остались в ветке-источнике), но сами гарантии — отказ устаревшей записи,
 * восстановление чекпойнта как новая ревизия, точный пофайловый diff —
 * принадлежат порту, а не конкретному хранилищу. Здесь они закреплены на
 * MemoryLayoutRepository: эталонной реализации LayoutRepositoryPort.
 */

function revised(
  document: LayoutDocument,
  changes: Partial<LayoutDocument["objects"][number]>,
): LayoutDocument {
...
describe("LS-050: version hardening", () => {
  it("rejects a stale saveDraft using expectedRevision", async () => {
    const repo = new MemoryLayoutRepository();
  ...
  it("restores a checkpoint as a new draft revision and leaves the checkpoint immutable", async () => {
    ...
    const restored = await repo.restoreCheckpoint(checkpoint.checkpointId, edited.stateRevision);
  ...
  it("rejects restoring a checkpoint over a draft that moved on", async () => {
    const repo = new MemoryLayoutRepository();
    const original = makeSimpleRoom();
    await repo.saveDraft(original, null);
    const checkpoint = await repo.createCheckpoint(original, {
      checkpointId: "checkpoint.simple-room.stale-restore",
      reasonCode: "USER_CHECKPOINT",
      reason: "До редактирования",
      createdAt: "2026-08-04T09:00:00.000Z",
    });
    const edited = revised(original, { xMm: 1400 });
    await repo.saveDraft(edited, original.stateRevision);

    await expect(
      repo.restoreCheckpoint(checkpoint.checkpointId, original.stateRevision),
