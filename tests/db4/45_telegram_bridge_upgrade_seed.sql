...
values
  ...
  ('54444444-4444-4444-8444-444444444444', '53333333-3333-4333-8333-333333333333',
   '51111111-1111-4111-8111-111111111111', 'owner_lead')
-- Членство могло появиться раньше: вставка `project_workflows` бутстрапит
-- владельца триггером. Фикстуре важно, что оно ЕСТЬ, а не кто его создал.
...
-- ДУБЛИКАТЫ, КОТОРЫЕ СТАРАЯ СХЕМА СЧИТАЛА ЗАКОННЫМИ.
--
-- Прежний индекс по чату включал `bot_instance_id`, поэтому две АКТИВНЫЕ связи
-- разных проектов в одном чате были нормой. Прежний индекс по проекту смотрел
-- только на `active`, поэтому рядом с активной спокойно жила `pending`. Оба
-- случая заводятся здесь настоящими строками: без них ветка схлопывания
-- миграции не исполняется ни разу, и «победитель детерминирован» остаётся
-- утверждением о непроверенном коде.

insert into public.projects (id, designer_id, client_name, status, intake_token)
values
  ('55555555-5555-4555-8555-555555555555',
   '51111111-1111-4111-8111-111111111111', 'Dup A', 'active_project', 'upgrade-dup-a'),
  ('56666666-6666-4666-8666-666666666666',
   '51111111-1111-4111-8111-111111111111', 'Dup B', 'active_project', 'upgrade-dup-b');

insert into project_intelligence.project_workflows (organization_id, project_id)
values
  ('54444444-4444-4444-8444-444444444444', '55555555-5555-4555-8555-555555555555'),
  ('54444444-4444-4444-8444-444444444444', '56666666-6666-4666-8666-666666666666');
...
-- Один чат, два проекта, два экземпляра бота — обе связи активны и обе
-- уведомлены. Победить обязана строка с БОЛЬШИМ временем активации; при равном
-- времени решает `binding_id`, поэтому оба поля здесь заданы явно.
insert into remhaos_channel.project_channel_bindings (
...
values
  ('54444444-4444-4444-8444-444444444444', '55555555-5555-4555-8555-555555555555',
   '5a000000-0000-4000-8000-00000000000a', 'telegram', 'upgrade-bot',
   -200300, 'supergroup', 'active', 'notice-v1',
   statement_timestamp() - interval '2 hours',
   '51111111-1111-4111-8111-111111111111', statement_timestamp() - interval '2 hours'),
  ('54444444-4444-4444-8444-444444444444', '56666666-6666-4666-8666-666666666666',
   '5b000000-0000-4000-8000-00000000000b', 'telegram', 'upgrade-bot-2',
   -200300, 'supergroup', 'active', 'notice-v1',
   statement_timestamp() - interval '1 hour',
   '51111111-1111-4111-8111-111111111111', statement_timestamp() - interval '1 hour');

-- Один проект, два чата: активная связь и оставшаяся рядом `pending`. Прежний
-- индекс по проекту смотрел только на `active` и такую пару пропускал.
insert into remhaos_channel.project_channel_bindings (
  organization_id, project_id, binding_id, provider, bot_instance_id,
  external_chat_id, external_chat_type, status, notice_version,
  notice_posted_at, initiated_by_user_id, activated_at
...
values
  ('54444444-4444-4444-8444-444444444444', '52222222-2222-4222-8222-222222222222',
   '5c000000-0000-4000-8000-00000000000c', 'telegram', 'upgrade-bot',
   -200400, 'supergroup', 'pending', 'notice-v1',
   null, '51111111-1111-4111-8111-111111111111', null);

do $duplicates_are_real$
begin
  if (
    ...
    raise exception 'DB4_TGU_SEED_MISSING_CHAT_DUPLICATE';
  end if;
  if (
    select count(*) from remhaos_channel.project_channel_bindings
    ...
    raise exception 'DB4_TGU_SEED_MISSING_PROJECT_DUPLICATE';
  end if;
end
$duplicates_are_real$;
...
begin
  ...
  -- Две «одиночные» активные связи плюс две активные в общем чате: четыре.
  if (select count(*) from remhaos_channel.project_channel_bindings where status = 'active') <> 4 then
    raise exception 'DB4_TGU_SEED_MISSING_ACTIVE_BINDINGS';
  end if;
  if (select count(*) from remhaos_channel.project_channel_bindings where status = 'pending') <> 1 then
    raise exception 'DB4_TGU_SEED_MISSING_PENDING_BINDING';
  end if;
end
$seed_is_real$;
