...
describe("M4 execution guardrail", () => {
  ...
  it("fails CI when any command is added to the contract without being classified", () => {
    ...
    expect(M3_SURFACE_COMMANDS.has("confirm_pdf_dwg_source_pair")).toBe(true);
    expect(EXECUTION_MODULE.has("confirm_pdf_dwg_source_pair")).toBe(false);
    expect(M3_SURFACE_COMMANDS.has("bind_pdf_dwg_sheet_sidecar")).toBe(true);
    expect(EXECUTION_MODULE.has("bind_pdf_dwg_sheet_sidecar")).toBe(false);
