...
-- одноразовый стек AP5, харнессы DB4 и AP1. В продакшене он не исполняется —
-- там модуль закрыт, и закрыт он отсутствием этих прав.
--
-- Обратная операция — повторно применить сами guardrail'ы (`revoke` в них
-- идемпотентен).
--
-- ЭТО НЕ PRODUCTION FEATURE FLAG (решение владельца 11.08 — оно принималось про
-- модуль 3, но правило общее). Production-включение M4 — отдельное решение, и
-- оно требует одного авторитетного DB-state с аудитом, поверх которого env-флаг
-- остаётся дополнительным hard-off.
