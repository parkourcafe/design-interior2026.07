...
describe("ProjectCEO command service", () => {
  it("returns a typed unavailable result before any partial invitation write", async () => {
    ...
    const result = await service.execute({
      ...
      projectId,
      payload: { recipientEmail: "architect@example.test", targetRole: "architect" },
    }, "invite-unavailable");
  ...
  it("keeps a stable DB idempotency key for an exact HTTP retry", async () => {
    ...
    const mutations = calls.filter((call) => call.functionName.endsWith("submit_change_request"));
    expect(mutations[0]?.args.idempotency_key).toBe(mutations[1]?.args.idempotency_key);
  ...
  it("derives the exact package from the selected version and rejects a sibling/unknown target", async () => {
    ...
    expect(calls.find((call) => call.functionName.endsWith("submit_change_request"))?.args)
      .toMatchObject({ package_id: packageA, from_baseline_id: baseline1, proposed_baseline_id: baseline2 });

    const before = calls.filter((call) => call.functionName.endsWith("submit_change_request")).length;
    ...
    expect(calls.filter((call) => call.functionName.endsWith("submit_change_request"))).toHaveLength(before);
  });
