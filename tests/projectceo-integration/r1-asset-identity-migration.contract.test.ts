...
describe("R1 immutable asset identity migration", () => {
  it("keeps every record private, append-only and pair-bound", () => {
    for (const table of ["external_assets", "external_asset_versions", "external_representation_versions", "external_representation_attestations", "external_asset_events"]) expect(sql).toContain(`projectceo_foundation.${table}`);
