...
**Статус на 02.08.2026:** пилот закрыт, окружение на паузе — итоги в §6.
...
```bash
zsh tests/ap1/environment/bootstrap-disposable.zsh --target local    # стек Supabase CLI
zsh tests/ap1/environment/bootstrap-disposable.zsh --target hosted   # managed-проект, env
```

> Историческая справка: 24.08.2026 существовало ДВЕ независимые реализации
> этого файла — из PR #100 и PR #101. В main влились обе (в #101 ребейз с
> удалением дубликата не состоялся), выжила версия #100 как более зрелая —
> решением конфликта в #100. Первая редакция этого раздела описывала
> интерфейс версии #101 (подкоманды `bootstrap`/`verify`/`receipt`,
> `AP1_TARGET`, токены `AP1_PRODUCTION_TARGET_REJECTED`,
> `AP1_NOT_CLEAN_BOOTSTRAP`) — этого интерфейса больше не существует.
> Раздел переписан под выживший скрипт; из уникальных файлов #101 живут
> `apply-hosted-role-precondition.sql` (standalone-предусловие §2a для
> SQL-редактора), шаблон отчёта и черновик решения владельца.

Скрипт делает ровно то, что описано ниже, и отказывается работать, если
предпосылка не выполнена. Дефекты развёртывания из §6 — отказы с именем:

| Отказ | Что означает |
|---|---|
| `AP1_PRODUCTION_REF_REJECTED` | ref прода в любой переменной окружения или аргументе (без учёта регистра) — §0 |
| `AP1_LEDGER_FOLDER_MISMATCH` | состав папки миграций разошёлся с реестром |
| `AP1_LEDGER_DIGEST_MISMATCH` | миграция изменена без обновления реестра хешей |
| `AP1_BOOTSTRAP_ROLE_MEMBERSHIP_INVALID` | нет членства `SET`/`INHERIT` — §2a |
| `AP1_AUTH_WORKAROUND_GRANT_PRESENT` | вернулся обход §2b — прогон доказал бы заплатку |
| `AP1_BOOTSTRAP_MIGRATION_FAILED <имя>` | падение файла цепочки; на непустой базе — с текстом guard'а baseline (§2, §2d) |

Обратное к последнему — строка `AP1_AUTH_WORKAROUND_GRANT_ABSENT` в выводе
шага ролей: вердикт по блокеру `92060e3` засчитывается только вместе с ней.

`--db-only` исполняет только шаги базы (роли → миграции по реестру →
постусловия `verify-db.sql`) и печатает итог как ЧАСТИЧНЫЙ;
`--skip-identities` пропускает пять ролевых личностей. Полный перечень
переменных окружения — в `--help`; детали и история — §7.

Чего скрипт НЕ делает: не создаёт hosted-проект, не настраивает Auth redirect
allowlist / SMTP / Storage-бакеты, не включает модули (DEC-029) и не выдаёт
членства ролевым пользователям. Первое и второе — владелец/оператор,
последнее — штатный invitation-поток (§3).
...
### Итог пилота (02.08.2026)

Окружение на момент закрытия: **14 миграций применены, 9 схем, 80 таблиц,
57 RPC**, создана 1 ревизия решения, `state_revision = 1`.

**Что доказано:**

| | |
|---|---|
| Слой ProjectCEO разворачивается на реальном Supabase | ✅ впервые проверено |
| Канонизация semantic hash совпадает с Postgres | ✅ побайтово, хэш закреплён в тестах |
| Read-контракт возвращает корректный scope | ✅ |
| Запись через RPC (`append_decision_revision`) | ✅ ревизия создана, state_revision 0 → 1 |
| Идемпотентность по ключу | ✅ повтор вернул `replay=true` без второй записи |
| Deny-by-default | ✅ `authenticated` не читает внутренние схемы |
| Замечания безопасности от слоя ProjectCEO | ✅ ноль (2 предупреждения — legacy `is_studio_member`) |

**Три дефекта развёртывания, найденные только на живой базе:**

1. Папка миграций не bootstrap-ится подряд — `legacy_production_baseline`
   заменяет `0001`–`0007`, а не продолжает их (§2).
2. Слой не ставится без ручной подготовки ролей `pi_*` — `postgres` в Supabase
   не superuser, PG16+ не выдаёт `SET`/`INHERIT` автоматически (§2a).
3. **Авторизация была сломана и падала молча** — `auth.uid()` внутри definer-
   функции, принадлежащей `pi_table_owner`, плюс три гранта, которые PostgreSQL
   молча не применял (§2b). Исправлено в PR #60 миграцией
   `20260801120000_projectceo_request_claim_authorization.sql`: RPC читают
   подписанные claims PostgREST вместо `auth.uid()`, доступ к схеме `auth`
   больше не нужен.

**Что осталось непроверенным:** сквозной `publish_baseline` (цепочка
approval package → submit → review → publish). Дошли до записи ревизии;
дальше требуется одобренный `ApprovalPackage`. Риск здесь низкий — самая
хрупкая часть, канонизация хэша, уже подтверждена отдельно.

**Также не проверено на живой базе:** фикс из PR #60. На момент закрытия
окружения он в AP1 не применялся — проверка шла с обходным грантом
`grant authenticated to pi_table_owner`. При следующем прогоне начинать
нужно с применения этой миграции и **снятия обходного гранта**, чтобы фикс
доказал себя без заплатки.

### Судьба окружения

Проект `uafvzxdxlxqkpsejgskt` поставлен **на паузу** 02.08.2026 — расход
остановлен. Полное удаление через MCP невозможно (инструмента нет, есть только
пауза/восстановление), поэтому удалять нужно вручную в дашборде Supabase.

Воспроизвести окружение с нуля можно по этому документу за один проход:
§2 порядок миграций → §2a роли → §2b (уже не нужен после PR #60) → §3 пользователи.

### Осталось решить

1. Судьба репозитория относительно прода (см. §0) — главная открытая развилка.
2. Убрать из миграций три гранта `grant usage on schema auth to pi_table_owner`:
   после PR #60 они не нужны и по-прежнему молча ничего не делают.
3. Решить по журналу миграций: пометить `0001`–`0007` как superseded либо
   задокументировать два пути bootstrap (§2).

---

## 7. Аддендум 24.08.2026 — стенд поднимается скриптом (Фаза 1)

Дополнение, а не правка: разделы 0–6 остаются как есть, включая то, что в них
устарело. Ниже сказано, что именно устарело и чем заменено.

### 7.1 Новый стандартный способ поднять стенд
...
Одна команда закрывает §2 (порядок миграций), §2a (роли `pi_*` и членства с
`inherit true, set true`), §4.3 (постусловия против возврата auth-дефекта) и §3
(пять ролевых личностей). Шаги в скрипте те же, что были расписаны текстом; чем
он отличается от чтения runbook руками:

* **порядок берётся из реестра, а не из папки.** Хеши
  `tests/ap1/environment/migration-ledger.sha256` сверяются ДО применения:
  лишний, переименованный или изменённый файл роняет прогон на предусловиях, а
  не на середине цепочки;
* **hosted-путь стал исполняемым.** До этого он существовал только как текст
  §2/§2a, и каждый прогон собирали заново. Три дефекта развёртывания из §6
  найдены ровно потому, что «собрать заново» и «собрать так же» — разные
  операции;
* **отказ от прода — первый шаг, а не осторожность.** Ref `ztnycrchwxqczqbyegnp`
  в любой переменной окружения или в любом аргументе даёт немедленный выход с
  `AP1_PRODUCTION_REF_REJECTED` — раньше разбора аргументов по существу;
* **компенсация ACL схемы `auth` измеряется, а не предполагается.** Скрипт
  спрашивает базу, применились ли исторические гранты (`grant usage on schema
  auth`), и отзывает их только там, где применились. На managed они инертны, и
  отзывать нечего — см. errata E-002;
* **повторный запуск идемпотентен.** Уже записанные в
  `supabase_migrations.schema_migrations` версии пропускаются; на непустой базе
  без журнала прогон падает guard'ом baseline с его собственным текстом
  («application relations already exist»), а не молча.

`run-local.zsh` остаётся: он про управление жизненным циклом локального стека
(`start`/`reset`/`restart`/`dispose`) на изолированном сокете colima и не
покрывает hosted.

### 7.2 Что в разделах выше устарело

* **§2, про папку миграций.** «В репозитории под номером `0007` лежит
  `invite_tokens`», «на чистом окружении применять baseline, а не 0001–0007» —
  описывает состояние до 01.08.2026. Файлов `0001`–`0009` в
  `supabase/migrations/` больше нет: они убраны из исполняемого входа
  (`AP1_MIGRATION_REPLAY_REPORT_2026-08-01.md`) и сохранены как байтовые
  доказательства в
  `docs/product-intelligence/agent-runs/db-wave/legacy-migrations/` с
  нерабочим расширением `.sql.txt`. Обе последовательности из §2 — историческая
  справка, а не инструкция.
* **§2d, `MIGRATION_ORDER_BLOCKED`.** Причина («CLI применяет `0001`–`0007`,
  затем baseline») снята тем же удалением: в папке остался ровно один путь.
  Прогон `supabase db push` на чистой managed-базе в этой фазе **не
  выполнялся** — снятие блокера подтверждено на уровне цепочки, не на уровне
  команды CLI.
* **§6, «Осталось решить», пункт 2** (три гранта `grant usage on schema auth`) —
  закрыт записью E-002 в `docs/canonical/remhaos-v1/REMHAOS_MIGRATION_ERRATA.md`
  и тестом, пришпиливающим набор из семи операторов.
* **§6, «Осталось решить», пункт 3** (журнал миграций, два пути) — закрыт
  `supabase/migrations/README.md`: решение теперь лежит там, где на него
  натыкаются, а не только в отдельных документах развилки.
* **§4.3, пункт 3.** Утверждение «и набор инертных `grant usage on schema auth`
  не растёт» с 10.08.2026 не было ничем обеспечено: контрактный тест смотрел
  только тела функций. Проверка добавлена 24.08.2026 (сценарий «pins the
  historical inert managed-auth grants»).

### 7.3 Старый проект `uafvzxdxlxqkpsejgskt`

Стоит на паузе с 02.08.2026, содержит состояние 14 миграций. Для Фазы 1 он **не
переиспользуется**: с тех пор цепочка выросла до 54 миграций (M2/M3/M4,
Telegram, V1 Impact), и распаковка паузы дала бы стенд с историей, которой на
чистом проекте не будет. Правильное действие — удалить его вручную в дашборде
Supabase (MCP умеет только паузу) и создать свежий disposable-проект.
