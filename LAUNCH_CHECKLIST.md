...
## Результат генерального аудита кода (09.07.2026)

| Проверка | Итог |
|---|---|
| Сборка: `build` / `lint` / `typecheck` / `test` (55) | ✅ зелёные |
| Production audit: `npm audit --omit=dev` | ✅ 0 vulnerabilities |
| RuStore release pack | ✅ `RUSTORE_RELEASE.md` |
...
| Кабинет `/dashboard` под защитой (Next proxy + guard) | ✅ |
...
- [ ] Для RuStore: создать аккаунт разработчика, выбрать package name, собрать APK/AAB, сохранить keystore, выставить `ANDROID_PACKAGE_NAME` и `ANDROID_CERT_SHA256` в Vercel.
- [ ] Для RuStore: подготовить тестовый аккаунт модератора, реальные скриншоты 9:16 и проверить рабочий email поддержки.
